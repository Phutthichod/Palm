...
switch ($request) {
  case 'insert':
    ...
    $sql = "INSERT INTO `db-pestlist` (`PID`, `Name`, `Alias`, `PTID`, `Charactor`, `Danger`, `lcon` , `NumPicChar`, `NumPicDanger`)
          VALUES ('','$Alias','$Name','2','$Charactor','$Danger','$Icon','$countfiles_style','$countfiles_danger')";
    ...
    for ($index_style = 0; $index_style < $countfiles_style; $index_style++) {
      if (move_uploaded_file($_FILES["picstyle_insert"]["tmp_name"][$index_style], "../../picture/Pest/disease/style/$id/$stylechar")) {
      }
      $stylechar = $index_style . "_" . $t . "." . $tmp_style;
    }
    for ($index_danger = 0; $index_danger < $countfiles_danger; $index_danger++) {
      if (move_uploaded_file($_FILES["picdan_insert"]["tmp_name"][$index_danger], "../../picture/Pest/disease/danger/$id/$dangerous")) {
      }
